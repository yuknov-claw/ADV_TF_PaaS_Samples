...
## ✅ Проверка связности перед работой

Перед созданием ресурсов проверьте, что Terraform с вашими credentials подключается к Cloud.ru:

```bash
cd connection-check/
cp ../k8s-cce/terraform.tfvars.example terraform.tfvars
nano terraform.tfvars          # вставьте свои access_key, secret_key
terraform init && terraform apply
```

Если в outputs `connection_status = ✅ Связность установлена` — всё готово.

## ❓ Типичные ошибки и их решение

### 1. `Error: Access key is missing`
**Причина:** Не заполнен `terraform.tfvars` или неверные credentials.
**Решение:** Проверьте `access_key` / `secret_key` в IAM консоли Cloud.ru Advanced.

### 2. `Error: Forbidden`
**Причина:** Не хватает прав IAM на сервис.
**Решение:** Выдайте нужную роль (см. таблицу выше) или обратитесь к администратору.

### 3. `Error: Conflict` при создании бакета
**Причина:** Имя уже занято.
**Решение:** Смените `bucket_name` в `terraform.tfvars`.

### 4. `Error: Quota exceeded`
**Причина:** Закончилась квота на ресурсы в проекте.
**Решение:** Запросите увеличение квоты через личный кабинет Cloud.ru.

### 5. Ресурс создан, но не виден в консоли
**Причина:** Неверный регион.
**Решение:** Убедитесь, что `region` в `terraform.tfvars` соответствует вашему проекту.

### 6. `terraform plan` показывает изменения для нетронутых ресурсов
**Причина:** Ресурс правили вручную через консоль.
**Решение:** Terraform вернёт его к состоянию конфига — убедитесь, что это ожидаемо.
